    ...
    def get_input(
        ...
        if rel_fnames:
            ...
            rel_read_only_stubs_fnames = [
                get_rel_fname(fname, root)
                for fname in (abs_read_only_stubs_fnames or [])
            ]
            show = self.format_files_for_input(rel_fnames, rel_read_only_fnames, rel_read_only_stubs_fnames)
...
{prefix} {hist}"""
    ...
    def format_files_for_input(
        self, rel_fnames, rel_read_only_fnames, rel_read_only_stubs_fnames
    ):
        if not self.pretty:
            lines = []
            ...
            for fname in sorted(rel_fnames):
                if (
                    fname not in rel_read_only_fnames
                    and fname not in rel_read_only_stubs_fnames
                ):
                    lines.append(fname)
            return "\n".join(lines) + "\n"
