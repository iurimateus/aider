...
class Commands:
    ...
    def cmd_tokens(self, args):
        ...
        for fname in self.coder.abs_read_only_stubs_fnames:
            ...
            if not is_image_file(relative_fname):
                ...
                tokens = self.coder.main_model.token_count(content)
                res.append(
                    (tokens, f"{relative_fname} (read-only stub)", "/drop to remove")
                )
    ...
    def completions_drop(self):
        ...
        read_only_files = [
            ...
            | self.coder.abs_read_only_stubs_fnames
        ]
    ...
    def _handle_read_only_files(self, expanded_word, file_set, description=""):
        ...
        for matched_file in matched:
            ...
            self.io.tool_output(
                f"Removed {description} file {matched_file} from the chat"
            )
    ...
    def cmd_ls(self, args):
        ...
        if (
            not chat_files
            and not other_files
            and not read_only_files
            and not read_only_stub_files
        ):
...
"""
    ...
    def cmd_cmd(self, args):
        ...
        if subcmd == "add":
            ...
            if len(words) != 2:
                self.io.tool_error(
                    "Usage: /cmd drop [command-name|path/to/commands.yaml]"
                )
                return
            target = words[1]
    ...
    def _cmd_read_only_base(
        self, args, source_set, target_set, source_mode, target_mode
    ):
        ...
        if not args.strip():
            ...
            for fname in list(self.coder.abs_fnames):
                ...
                self.io.tool_output(
                    f"Converted {rel_fname} from editable to {target_mode}"
                )
        ...
        for path in sorted(all_paths):
            ...
            if os.path.isfile(abs_path):
                ...
                self._add_read_only_directory(
                    abs_path, path, source_set, target_set, target_mode
                )
            else:
    ...
    def _add_read_only_file(
        ...
        if abs_path in target_set:
            self.io.tool_error(
                f"{original_name} is already in the chat as a {target_mode} file"
            )
            return
...
class UserCommandRegistry:
    ...
    def list_commands(self, io):
        ...
        max_name_len = max(
            (len(name) for cmds in by_source.values() for name, _ in cmds), default=20
        )
